...
class Rating {
	...
	public function return_success( $arr ) {
		...
		if ($arr['count'] > 0) {
            $result2 = $this->db->insert_or_replace_overall(
                array(
                    'item' => $this->item,
                    'average' => $arr['overall_rating'],
                    'ratings' => $arr['count']
                )
            );
            if ($result2[0] === true) {
                $arr['additional_msg'] = 'recording overall rating succeeded';
            } else {
                $arr['additional_msg'] = 'recording overall rating failed';
                $arr['additional_error'] = $result2[1] . ': ' . $result2[2];
            }
        }
		$this->return_json( $arr );
		exit();
